...
class Doctor(Agent):
    def __init__(self, engine=None, doctor_info=None, name="A"):
        if doctor_info is None:
            self.system_message = \
                "You are a professional and patient chronic disease doctor, there will be sub-healthy obese patients to consult you. You need:\n" + \
                "(1) Do not make a premature diagnosis without sufficient information.\n" + \
                "(2) Ask the patient multiple, proactive questions to obtain sufficient information, such as their diet, life status..\n" + \
                "(3) Ask one question at a time and keep it short. \n" + \
                "(4) Finally, according to the patient's physical condition, the diagnosis result, the corresponding diagnosis basis and treatment plan are given. \n" + \
                "(5) Also outline the potential complications that resident may encounter."
        else: self.system_message = doctor_info

        self.name = name

        self.doctor_greet = "Hello, what's the problem with your health?"
        self.engine = engine
    ...
    def get_diagnosis_by_patient_id(self, ID, key="ALL"):
        if key == "ALL":
            return self.diagnosis[ID]
        else:
            assert key in ["symptoms", "diagnosis", "treatment", "possible complications"]
            return self.diagnosis[ID].get(key)
